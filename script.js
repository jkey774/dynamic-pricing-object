(function() {
  ...
  var pricingObj = {
    careLevels: (function() {
      var careLevels = digitalData.page.pageInfo.levelsOfCare.slice(0);
      var _careLevels = {};
      for (var i = careLevels.length - 1; i >= 0; --i) {
        var careLevel = careLevels[i],
            careId = careLevel.care_type_id;
        if (careId === "IL" || careId === "AL" || careId === "MC") {
          _careLevels[careId] = careLevel;
        }
      }
      return _careLevels;
    })(),
    careProfiles: [],
    setCareProfiles: function(careId) {
      pricingObj.careProfiles.push([
        careId,
        pricingObj.careLevels[careId].name,
        pricingObj.price.careIncluded[careId],
        pricingObj.strConstants.careIconLinkTails[careId],
        pricingObj.strConstants.careDescriptions[careId][0],
        parseInt(pricingObj.price[careId], 10).toLocaleString("en"),
        pricingObj.strConstants.careDescriptions[careId][1]
      ]);
    },
    price: (function() {
    ...
    domConstants: {
      ...
      pricingFooter: document.querySelector(
        ".pricing-info__footer-community-specials"
      )
    },
    contentRendering: {
      ...
      renderHeader: function() {
        pricingObj.domConstants.communityName.insertAdjacentHTML(
          "beforeEnd",
          digitalData.page.pageInfo.pageName
        );
        pricingObj.domConstants.date.insertAdjacentHTML(
          "beforeEnd",
          "As of " + pricingObj.contentRendering.getLastBusinessDay()
        );
      },
      ...
      checkLargeAlMcPriceGap: function() {
        if (
          pricingObj.price.AL &&
          pricingObj.price.MC - pricingObj.price.AL > 1500
        ) {
          return true;
        }
      },
      ...
      renderPrice: function(price, careId) {
        if (
          ...
          price < 500 ||
          (careId === "MC" &&
            pricingObj.contentRendering.checkLargeAlMcPriceGap() === true)
        ) {
      ...
      checkIfMedicaid: function() {
        if (pricingObj.price.medicaid === "1") {
          pricingObj.domConstants.pricingFooter.insertAdjacentHTML(
            "beforeEnd",
            pricingObj.strConstants.communitySpecials.medicaid
          );
        }
      },
      // check whether semiprivate rooms are available at the community
      checkIfSemiPrivate: function() {
        if (pricingObj.price.semiPrivate === "1") {
          pricingObj.domConstants.pricingFooter.insertAdjacentHTML(
            "beforeEnd",
            pricingObj.strConstants.communitySpecials.semiPrivate
          );
        }
      }
    },
    init: function() {
      pricingObj.contentRendering.renderHeader();
      pricingObj.contentRendering.checkIfMedicaid();
      pricingObj.contentRendering.checkIfSemiPrivate();
      // create careProfile arrays for each set of care level data
      for (var key in pricingObj.careLevels) {
        if (!pricingObj.careLevels.hasOwnProperty(key)) {
          continue;
        }
        if (key === "IL" || key === "AL" || key === "MC") {
          pricingObj.setCareProfiles(pricingObj.careLevels[key].care_type_id);
        }
      }
      ...
      function createPriceCard(
        ...
        return {
          ...
          render: function() {
            pricingObj.domConstants.pricingRow.insertAdjacentHTML(
              "beforeEnd",
              '<div class="col-md-4 text-center pricing-info__care-included-' +
                pricingObj.contentRendering.getCareIncludedClassNum(careIncluded) +
                '" id="pricingBlock' +
                careId +
                ...
                pricingObj.contentRendering.renderPrice(price, careId) +
                '</div><div class="pricing-info__item--footer"><p>' +
                descriptionB +
                "</p>" +
                pricingObj.strConstants.svgLogo +
                "</div></div></div>" +
                pricingObj.contentRendering.checkIfCareIncluded(careIncluded) +
                "</div>"
            );
          }
        };
      ...
      for (var i = 0, len = pricingObj.careProfiles.length; i < len; i++) {
        createPriceCard.apply({}, pricingObj.careProfiles[i]).render();
      }
    }
  };
  pricingObj.init();
